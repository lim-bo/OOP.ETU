...
Communicator::Communicator(CommParams& pars, QWidget* parent) : QUdpSocket(parent) {
    par = pars;
    readyToRecieve = bind(par.recieverHost, par.recieverPort,
...
void Communicator::send(QByteArray data) {
    if (readyToRecieve) {
        writeDatagram(data, par.senderHost, par.senderPort);
    }
}
